(function () {
  ...
  function playRandomAudio(dir) {
    ...
    // 随机选择一个音频对象
    // 注意：如果是同一个Audio对象，正在播放时再次调用play可能会重置或报错，
    // 但这里我们有多个文件，且为了简单起见，我们直接克隆或者重新new一个（利用浏览器缓存）
    // 或者更好的方式：如果缓存的audio正在播放，使用cloneNode
    
    const originalAudio = audios[Math.floor(Math.random() * audios.length)];
    // 使用 cloneNode() 可以实现重叠播放，且利用缓存
